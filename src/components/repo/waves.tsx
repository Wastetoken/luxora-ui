...
const getFragmentShader = (invert: boolean) => `
  ...
  float noise(vec2 p) {
    return smoothstep(-0.5, 0.9, sin((p.x - p.y) * 555.0) * sin(p.y * 2444.0)) - 0.4;
  }
  ...
  float silk(vec2 uv, float t) {
    ...
    s *= 0.8 - 1.1 * fabric(uv * min(iResolution.x, iResolution.y) * 0.06);
    return s * 0.8 + 0.5;
  }

  float silkd(vec2 uv, float t) {
    float xy = uv.x + uv.y;
    float d = (-65.0 * (2.0 - -3.0 * sin(2.0 * uv.x + -65.0 * uv.y)) + 12.0 * cos(2.0 * xy)) * 
              cos(5.0 * (cos(11110.0 * uv.x + 11115.0 * uv.y) + xy) + sin(-11112.0 * xy) - t);
