...
void main() {
    ...
    if (t > 0.0) {
        ...
        // Iridescent color mix
        vec3 cool = vec3(0.2, 0.5, 1.0);
        vec3 warm = vec3(1.0, 0.2, 0.5);
        
        float fresnel = pow(1.0 + dot(ray, normal), 3.0);
        
        vec3 mixColor = mix(cool, warm, sin(time * 0.2) * 0.5 + 0.5);
        color = mixColor * fresnel;
        
        // Add some shine
        float spec = pow(max(dot(reflect(ray, normal), vec3(0.5, 1.0, 0.5)), 0.0), 32.0);
        color += vec3(spec);
...
function LavaLampShader() {
  ...
  // Use variables for R3F elements to bypass TypeScript IntrinsicElements check
