...
const FluidInversionCursor = ({
  ...
  pressure = 1.0,
  pressureIterations = 4000,
  curl: curlStrength = 105,
  splatRadius = 0.12,
  splatForce = 1000,
  cursorSize = 100,
  className,
  ...
  useEffect(() => {
    ...
    if (!canvas || !dot || !container) return;
    ...
    const handleMouseLeave = () => { pointer.down = false; dot.style.opacity = "0"; };
    const handleMouseEnter = () => { pointer.down = true; dot.style.opacity = "1"; };
    ...
    function update() {
      ...
      if (!CFG.PAUSED) {
        if (pointer.moved) {
          pointer.moved = false;
          splatFn(pointer.x, pointer.y, pointer.dx * CFG.SPLAT_FORCE / 1000, pointer.dy * CFG.SPLAT_FORCE / 1000, color);
        }
        step(dt);
        render();
      }
